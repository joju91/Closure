...
| T118 | Programmatisk SEO — `seo/cities.json` (20 städer, namn + län), generator `scripts/generate-city-pages.cjs`, 40 statiska sidor (`bouppteckning-[stad].html` + `dodsfallsintyg-[stad].html` × 20 städer) med FAQPage-schema, sitemap uppdaterad, interna länkar tillagda i `bouppteckning-guide.html` och `dodsfallsintyg.html`. Stadssidor länkar till domstol.se för uppslag av lokal tingsrätt. | 2026-05-17 | Fas 12 | SEO & Growth | 🟠 | SEO | ✔ |
...
| T121 | Topical authority map — 5 kluster med pillar + befintliga och nya stödsidor, ~50 nyckelord identifierade. Innehåller mall + process (sökvolymer kräver Ahrefs/GSC innan publiceringsplan kan beslutas). Leverans: `seo/topical-authority-map.md`. | 2026-05-17 | Fas 12 | SEO & Growth | 🟠 | SEO | ✔ |
| T122 | BreadcrumbList JSON-LD tillagd på 42 sidor som saknade det (gap från T120-siteaudit). Skript `scripts/add-breadcrumbs.cjs` extraherar h1 + canonical och infogar `BreadcrumbList`-schema före `</head>`. Resultat: 74/74 publika sidor har breadcrumb-schema. | 2026-05-17 | Fas 12 | T120-uppföljning | 🟠 | SEO | ✔ |
| T123 | `vad-gora-nar-nagon-dor.html` optimerad enligt T116-brief: 4 nya FAQ-entries i schema + motsvarande synliga H3-stycken (dör hemma, agera å dödsboets vägnar, hittar inte testamente, dör utomlands), HowTo-schema med 10 steg för list-snippet-targeting, H2 ovanför 10-stegslistan omformulerad ("Vad ska man göra när någon dör? — 10 steg i rätt ordning"), dateModified uppdaterad till 2026-05-17. | 2026-05-17 | Fas 12 | T116-uppföljning | 🟠 | SEO | ✔ |
| T124 | `bouppteckning-guide.html` optimerad: 3 nya FAQ-entries (missad tidsgräns, dödsbodelägare närvara, digitala tillgångar i bouppteckning) — alla med svar som motsvarar redan befintligt innehåll på sidan. dateModified uppdaterad. Intro-text till stadsidor-listan korrigerad (tog bort vilseledande "tingsrätt och kostnad" — stadsidorna länkar istället till domstol.se). | 2026-05-17 | Fas 12 | T117-uppföljning | 🟠 | SEO | ✔ |
| T125 | UI-pris ändrat från 149 kr → 49 kr i `index.html` (paywall-CTA), `app.js` (preview-CTA), `hjalp-med-dodsbo.html` (titel, meta description, og:description, Product-schema, visible copy). **Blocker:** Stripe-priset styrs av `STRIPE_PRICE_ID` env-var i Vercel — pekar fortfarande på 149-kr-Price-objekt. Innan branchen mergas och deploas: skapa ny 49-kr Price i Stripe Dashboard och uppdatera `STRIPE_PRICE_ID` i Vercel production+preview. Tills dess: UI 49 kr / kassa 149 kr. | 2026-05-17 | Fas 12 | Pricing | 🔴 | Dev | ⧖ |
| T126 | Fraunces + IBM Plex Sans fonts tillagda på 69 sidor som saknade dem (gap från T120-siteaudit). Skript `scripts/add-fonts.cjs` infogar preconnect + preload-block (matchar exakt det 5 befintliga sidor använder) före `style.css`. Resultat: 74/74 publika sidor har samma font-stack. | 2026-05-17 | Fas 12 | T120-uppföljning | 🟠 | Design | ✔ |
| T127 | dateModified-hook (T120-uppföljning): **beslut skipas**. Med 74 sidor och oregelbundet redigeringstempo blir en pre-commit-hook mer komplexitet än värde (risk för felaktiga ändringar på filer som ej redigerats, kräver re-install vid varje clone). Lösning: uppdatera `dateModified` manuellt vid varje content-ändring i berörd HTML. | 2026-05-17 | Fas 12 | T120-uppföljning | 🟢 | Process | x |
